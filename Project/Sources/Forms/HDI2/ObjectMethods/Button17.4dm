...
If ($target#Null:C1517)
	...
	WP COMPUTE FORMULAS:C1707($target)
...
Else 
	...
	For each ($expression;Form:C1466.formulas)
		...
		If ($expression.range#Null:C1517)
			WP COMPUTE FORMULAS:C1707($expression.range)
		Else 
			
			If ($expression.anchoredID#Null:C1517)
				$picture:=WP Get element by ID:C1549(WParea;$expression.anchoredID)
				WP COMPUTE FORMULAS:C1707($picture)
			Else 
